...
const { renderProfile, renderGalleries, renderRecentPosts, loadProfile, loadGalleries, loadRecentPosts } = require('../profile');
...
describe('renderGalleries function', () => {
    ...
    it('renders galleries', () => {
        renderGalleries(profile.albums);

        const albumContainer = document.querySelector('#Album');
        const galleryContainer = document.querySelector('#Gallery');

        expect(albumContainer.innerHTML).toContain(profile.albums[0].title);
        expect(albumContainer.innerHTML).toContain(profile.albums[0].photoCount);

        expect(galleryContainer.innerHTML).toContain(profile.albums[0].thumbnailUrl);
    });
});

describe('renderRecentPosts function', () => {
        ...
        it('renders recent posts', () => {
            renderRecentPosts(profile.recentPosts);
    
            const postsContainer = document.querySelector('#Posts');
    
            expect(postsContainer.innerHTML).toContain(profile.recentPosts[0].user);
            expect(postsContainer.innerHTML).toContain(profile.recentPosts[0].title);
            expect(postsContainer.innerHTML).toContain(profile.recentPosts[0].body);
        });
});
...
describe('loadGalleries function', () => {
        ...
        beforeEach(() => {
            fetch = jest.fn(() => Promise.resolve({
                json: () => Promise.resolve(profile.albums)
            }));
            document.body.innerHTML = `
                <div id="Album"></div>
                <div id="Gallery"></div>
        ...
        it('loads galleries', async () => {
            ...
            expect(fetch).toBeCalledWith(`http://localhost:3001/albums/${userId}`);
        });
});

describe('loadRecentPosts function', () => {
